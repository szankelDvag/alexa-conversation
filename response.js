...
function initResponse(test, conversationApi, fixSpaces, fuzzyDefault, isSsml) {
    ...
    const responseApi = {
        ...
        shouldEqualOne: (expected, reprompt) => shouldEqualOne(true, expected, reprompt),
    ...
    function shouldEqualOne(shouldOrNot, expected, expectedReprompt) {
        test.checks.push(testCase => {
            const actuals = getActuals(testCase);
            if (paramPassed(expected)) {
                it(`Alexa's ${type} response should ${not(shouldOrNot)}equal one of an array of answers`, () =>{
                    let expectedArray = expected();
                    expectedArray = typeof expectedArray === 'string' ? [expectedArray] : expectedArray;
                    let map =  _.without(_.map(expectedArray, (expectedSpeech) => {
            ...
            if (paramPassed(expectedReprompt)) {
                it(`Alexa's ${type} reprompt should ${not(shouldOrNot)}equal one of an array of answers`, () => {
